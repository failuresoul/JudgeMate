...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-slate-800 pb-5">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-white">Problems</h1>
            <p class="mt-1 text-sm text-slate-400">Practice your coding skills by solving problems.</p>
        </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-800/60">
                        ...
                        <tr class="group hover:bg-slate-800/30 transition-colors duration-100">
                            {{-- Title --}}
                            <td class="px-6 py-4">
                                <a href="{{ route('problems.show', $problem) }}" class="font-semibold text-slate-100 hover:text-indigo-400 transition-colors text-base">
                                    {{ $problem->title }}
                                </a>
                            ...
                            <td class="px-6 py-4 text-slate-300">
                                <div class="flex items-center gap-2">
                                    <div class="h-6 w-6 rounded-md bg-indigo-500/10 text-indigo-400 flex items-center justify-center font-bold text-[10px]">
                            ...
                            <td class="px-6 py-4">
                                @if($problem->is_published)
                                    <span class="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2.5 py-0.5 text-xs font-semibold text-emerald-400 ring-1 ring-emerald-500/20">
                                        Published
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 rounded-full bg-slate-500/10 px-2.5 py-0.5 text-xs font-semibold text-slate-400 ring-1 ring-slate-500/20">
                                        Draft
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('problems.edit', $problem) }}" 
                                       class="inline-flex items-center gap-1 rounded-lg bg-slate-800 border border-slate-700 px-2.5 py-1.5 text-xs font-semibold text-slate-300 hover:bg-slate-700 hover:text-white transition-colors duration-150">
                                        Edit
                                    </a>
                                    <form method="POST" action="{{ route('problems.destroy', $problem) }}" class="inline-block">
                                        ...
                                        <button type="submit" 
                                                ...
                                                class="inline-flex items-center gap-1 rounded-lg bg-rose-500/10 border border-rose-500/20 px-2.5 py-1.5 text-xs font-semibold text-rose-400 hover:bg-rose-500/20 transition-colors duration-150">
                                            Delete
                                        </button>
                                    </form>
                                </div>
