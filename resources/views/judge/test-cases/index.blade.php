...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Test Cases</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Select a problem to add, modify, or delete its evaluation test cases.</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 overflow-hidden shadow-sm dark:shadow-xl">
        @if($problems->isEmpty())
            <div class="flex flex-col items-center justify-center gap-3 py-16 text-center">
                ...
                <p class="text-sm text-slate-600 dark:text-slate-500">You haven't created any problems yet.</p>
                <a href="{{ route('problems.create') }}" class="mt-2 text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 transition-colors">Create your first problem</a>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-200 dark:border-slate-800 text-left">
                            <th class="px-6 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Problem</th>
                            <th class="px-6 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Difficulty</th>
                            <th class="px-6 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Visible / Sample</th>
                            <th class="px-6 py-3 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Hidden / Evaluator</th>
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800/60">
                        ...
                        <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors duration-100">
                            {{-- Title & Slug --}}
                            <td class="px-6 py-4">
                                <a href="{{ route('judge.test-cases.show', $problem) }}" class="font-semibold text-slate-900 dark:text-slate-100 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors text-base">
                                    {{ $problem->title }}
                                </a>
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-full bg-emerald-100 dark:bg-emerald-500/10 px-2.5 py-0.5 text-xs font-medium text-emerald-700 dark:text-emerald-400">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-full bg-amber-100 dark:bg-amber-500/10 px-2.5 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-400">
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('judge.test-cases.show', $problem) }}" 
                                   class="inline-flex items-center gap-1 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 px-3 py-1.5 text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 transition-colors duration-150">
                                    Manage Test Cases
                                </a>
                            </td>
                        </tr>
