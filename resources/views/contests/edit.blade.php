...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Edit Contest</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Update contest details or change assigned problems.</p>
        </div>
    ...
    {{-- Form --}}
    <form method="POST" action="{{ route('contests.update', $contest) }}" class="space-y-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 p-6 shadow-sm dark:shadow-xl">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                      class="mt-1.5 block w-full rounded-lg border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-950 px-4 py-2.5 text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 transition-colors focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">{{ old('description', $contest->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <label for="is_active" class="text-sm font-semibold text-slate-700 dark:text-slate-200">Activate Contest</label>
        </div>
