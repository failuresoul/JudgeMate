<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-2xl border border-slate-800 bg-slate-950/40 backdrop-blur-xl shadow-lg hover:border-slate-700/60 transition-all duration-200 group">
    {{-- Left column: Title & Metadata --}}
    <div class="space-y-2">
        <div class="flex items-center gap-2.5">
            <a href="{{ route('contests.show', $contest) }}" class="text-lg font-bold text-slate-100 hover:text-indigo-400 transition-colors">
                {{ $contest->title }}
            </a>
        ...
        {{-- Metadata badges --}}
        <div class="flex flex-wrap items-center gap-y-1.5 gap-x-4 text-xs text-slate-400">
            <span class="flex items-center gap-1">
                ...
                By <span class="font-medium text-slate-300">{{ $contest->creator->name ?? 'System' }}</span>
            </span>
            <span class="flex items-center gap-1">
                <svg class="h-3.5 w-3.5 text-slate-550" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div class="flex items-center gap-4 text-sm text-slate-400">
        <div class="flex flex-col">
            ...
            <span class="text-slate-200 font-mono text-xs mt-1">
                @if($type === 'active')
    ...
    <div class="flex flex-wrap items-center gap-2 self-start md:self-center">
        ...
        @if(!$contest->is_approved)
            ...
            <form method="POST" action="{{ route('contests.approve', $contest) }}" class="inline-block">
                @csrf
                <button type="submit" 
                        class="inline-flex items-center gap-1 rounded-xl bg-emerald-500/10 border border-emerald-500/25 px-4 py-2 text-xs font-bold text-emerald-400 hover:bg-emerald-500/20 hover:border-emerald-500/40 transition-colors duration-150">
                    Approve
                </button>
            </form>
            @endrole
        ...
        @if($contest->is_approved)
            @if(auth()->user()->hasRole('Contestant'))
                ...
                @else
                    <form method="POST" action="{{ route('contests.register', $contest) }}" class="inline-block">
                        @csrf
                        <button type="submit" 
                                class="inline-flex items-center gap-1 rounded-xl bg-indigo-600/15 border border-indigo-500/25 px-4 py-2 text-xs font-bold text-indigo-400 hover:bg-indigo-600/30 hover:border-indigo-500/40 transition-colors duration-150">
                            Register
                        </button>
                    </form>
                @endif
            @else
                <a href="{{ route('contests.show', $contest) }}" 
                   class="inline-flex items-center gap-1 rounded-xl bg-slate-800 border border-slate-700 px-4 py-2 text-xs font-bold text-slate-300 hover:bg-slate-700 hover:text-white transition-all duration-150">
                    Enter
                </a>
            @endif
        @else
        ...
        @if(auth()->id() === $contest->created_by || auth()->user()->hasRole('Admin'))
            ...
            <form method="POST" action="{{ route('contests.destroy', $contest) }}" class="inline-block">
                ...
                <button type="submit" 
                        ...
                        class="inline-flex items-center gap-1 rounded-xl bg-rose-500/10 border border-rose-500/20 px-4 py-2 text-xs font-bold text-rose-400 hover:bg-rose-500/20 hover:border-rose-500/40 transition-colors duration-150">
                    Delete
                </button>
            </form>
        @endif
