...
<div class="space-y-6 max-w-5xl mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/10 shadow-sm p-6 space-y-4">
                <div>
                    <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">{{ $contest->title }}</h1>
            ...
            {{-- Problems --}}
            <div>
                <h2 class="text-xl font-bold text-slate-800 dark:text-white mb-4">Contest Problems</h2>
                @php
                    $now = now();
                        ...
                        @else
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm text-left text-slate-700 dark:text-slate-300">
                                    ...
                                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800/60 bg-white dark:bg-slate-950/20">
                                        @foreach($contest->problems as $problem)
                                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/20 transition-colors">
                                                <td class="px-4 py-3 whitespace-nowrap font-mono text-indigo-600 dark:text-indigo-400 font-bold text-lg">
                                                    {{ $problem->pivot->label ?? '?' }}
                                                </td>
                                                <td class="px-4 py-3 font-semibold text-slate-800 dark:text-slate-200">
                                                    <a href="{{ route('problems.show', [$problem, 'contest_id' => $contest->id]) }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors text-base font-bold">
                                                        {{ $problem->title }}
                                                    </a>
                                                </td>
                ...
                @else
                    <div class="rounded-2xl border border-slate-200 dark:border-slate-850 bg-white dark:bg-slate-950/40 p-8 text-center space-y-4 shadow-xl">
                        <div class="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-indigo-600 dark:text-indigo-400">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                            </svg>
                        </div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-slate-200">Problems are Locked</h3>
                        ...
                        <div class="text-xs text-indigo-600 dark:text-indigo-400 font-mono">Starts {{ $contest->starts_at->diffForHumans() }}</div>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 shadow-sm p-6 space-y-5">
                <h3 class="text-sm font-bold uppercase tracking-widest text-indigo-500 dark:text-indigo-400">Contest Info</h3>
                <div class="space-y-3">
                    <div>
            ...
            @if($hasStarted)
                ...
                <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 shadow-sm p-6 space-y-4">
                    <div class="flex items-center justify-between">
                        <h3 class="text-sm font-bold uppercase tracking-widest text-indigo-500 dark:text-indigo-400">Leaderboard</h3>
                        <span class="inline-flex items-center rounded bg-indigo-50 dark:bg-indigo-500/10 px-1.5 py-0.5 text-[10px] font-bold text-indigo-600 dark:text-indigo-400 ring-1 ring-indigo-500/20">Standings</span>
                    </div>
                    @if(empty($scoreboardStandings))
                        <p class="text-xs text-slate-500">No standings computed yet.</p>
                    @else
                        <div class="space-y-3 max-h-68 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60">
                            @foreach($scoreboardStandings as $index => $standing)
                                <div class="flex items-center justify-between text-xs py-2 {{ $index > 0 ? 'border-t border-slate-100 dark:border-slate-900/30' : '' }}">
                                    <div class="flex items-center gap-2">
                                        <span class="font-mono font-bold text-slate-400 dark:text-slate-500 w-4">{{ $index + 1 }}</span>
                                        <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $standing['name'] }}</span>
                                    </div>
                                    <div class="flex items-center gap-2.5 font-mono text-[10px]">
                                        <span class="text-emerald-600 dark:text-emerald-400 font-bold" title="Solved Problems">{{ $standing['solve_count'] }} AC</span>
            ...
            @else
                {{-- Participants List --}}
                <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 shadow-sm p-6 space-y-4">
                    <h3 class="text-sm font-bold uppercase tracking-widest text-indigo-500 dark:text-indigo-400">Participants</h3>
                        ...
                        <div class="space-y-3 max-h-60 overflow-y-auto">
                            @foreach($contest->participants as $participant)
                                <div class="flex items-center justify-between text-xs py-1">
                                    <span class="font-medium text-slate-700 dark:text-slate-300">{{ $participant->name }}</span>
