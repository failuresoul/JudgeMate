...
<div class="space-y-6 max-w-6xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-extrabold text-white tracking-tight mt-1">Live ICPC Scoreboard</h1>
            <p class="text-xs text-slate-400 mt-1">Real-time standings for <span class="text-slate-200 font-semibold">{{ $contest->title }}</span>.</p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-800 bg-slate-900/30 overflow-hidden shadow-xl">
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-slate-300 min-w-[700px]">
    ...
    const renderScoreboard = (data) => {
            ...
            scoreboardBody.innerHTML = `
                <tr>
                    <td colspan="${4 + data.problems.length}" class="px-4 py-12 text-center text-slate-500">
                        ...
                        <p class="text-sm font-semibold text-slate-400">No participants enrolled</p>
        ...
        data.rows.forEach((row, index) => {
            ...
            if (rank === 1) {
                ...
                rankBadge = '<span class="inline-flex items-center justify-center h-5 w-5 rounded-full bg-amber-400/10 text-amber-400 text-[10px] font-bold">🥇</span>';
            } else if (rank === 2) {
                rankClass = 'text-slate-400 font-black';
                rankBadge = '<span class="inline-flex items-center justify-center h-5 w-5 rounded-full bg-slate-400/10 text-slate-400 text-[10px] font-bold">🥈</span>';
            } else if (rank === 3) {
                rankClass = 'text-amber-600 font-black';
                rankBadge = '<span class="inline-flex items-center justify-center h-5 w-5 rounded-full bg-amber-600/10 text-amber-600 text-[10px] font-bold">🥉</span>';
            }
            ...
            data.problems.forEach(problem => {
                ...
                if (!detail) {
                    ...
                    const attemptsText = detail.wrong_attempts > 0 ? `<div class="text-[10px] text-emerald-400/80 font-bold">+${detail.wrong_attempts}</div>` : '';
                    problemsCells += `
                        <td class="px-4 py-3 text-center bg-emerald-500/5 border border-emerald-500/15">
                            <div class="font-mono text-emerald-400 font-semibold text-xs">${detail.minutes}m</div>
                            ${attemptsText}
                        </td>
                    `;
                } else {
            ...
            html += `
                <tr class="hover:bg-slate-900/20 transition-colors">
                    ...
                    <td class="px-4 py-3 text-center">
                        <span class="inline-flex items-center rounded-full bg-indigo-500/15 px-2.5 py-0.5 text-xs font-bold text-indigo-400 ring-1 ring-indigo-500/20">
                            ${row.solve_count}
                        </span>
                    </td>
