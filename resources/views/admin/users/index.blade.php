...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <span class="inline-flex items-center gap-1.5 rounded-lg bg-red-500/10 px-3 py-1.5 text-xs font-semibold text-red-400 ring-1 ring-red-500/20 self-start sm:self-auto">
            <svg class="h-3.5 w-3.5 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z" clip-rule="evenodd"/></svg>
            Admin Panel
        </span>
    </div>
    ...
    {{-- Status Tabs --}}
    <div class="flex overflow-x-auto gap-1 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-1">
        @foreach(['pending' => ['label'=>'Pending','color'=>'amber'], 'approved' => ['label'=>'Approved','color'=>'emerald'], 'rejected' => ['label'=>'Rejected','color'=>'red'], 'all' => ['label'=>'All Users','color'=>'slate']] as $key => $tab)
            <a href="{{ route('admin.users.index', ['filter' => $key]) }}"
               class="whitespace-nowrap shrink-0 flex-1 flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-all duration-150
                      {{ $filter === $key ? 'bg-white border border-slate-200 dark:border-slate-700 dark:bg-slate-800 text-slate-900 dark:text-white shadow' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-slate-200' }}">
                {{ $tab['label'] }}
                @if(isset($counts[$key]))
                    <span class="rounded-md px-1.5 py-0.5 text-xs font-bold {{ $counts[$key] > 0 && $key === 'pending' ? 'text-amber-700 dark:text-amber-400 bg-amber-100 dark:bg-amber-500/20' : 'text-slate-700 dark:text-slate-300 bg-slate-200 dark:bg-slate-700/60' }}">
                        {{ $counts[$key] }}
                    </span>
                @endif
            </a>
    ...
    <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 overflow-hidden shadow-sm dark:shadow-xl">
        @if($users->isEmpty())
            <div class="flex flex-col items-center justify-center gap-3 py-16 text-center">
                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-white border border-slate-200 dark:border-slate-700 dark:bg-slate-800">
                            ...
                            <td class="px-6 py-4">
                                @foreach($user->roles as $role)
                                    @php
                                        ...
                                        $color = $roleColors[$role->name] ?? 'text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-700/50 ring-slate-200 dark:ring-slate-600/30';
                                    @endphp
