...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-400">Review and approve registration requests.</p>
        </div>
        ...
        @foreach(['pending' => ['label'=>'Pending','color'=>'amber'], 'approved' => ['label'=>'Approved','color'=>'emerald'], 'rejected' => ['label'=>'Rejected','color'=>'red'], 'all' => ['label'=>'All Users','color'=>'slate']] as $key => $tab)
            <a href="{{ route('admin.users.index', ['filter' => $key]) }}"
               class="flex-1 flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-all duration-150
                      {{ $filter === $key ? 'bg-slate-800 text-white shadow' : 'text-slate-400 hover:text-slate-200' }}">
                {{ $tab['label'] }}
                @if(isset($counts[$key]))
                    <span class="rounded-md bg-slate-700/60 px-1.5 py-0.5 text-xs font-bold {{ $counts[$key] > 0 && $key === 'pending' ? 'text-amber-400' : 'text-slate-300' }}">
                        {{ $counts[$key] }}
                    </span>
                @endif
            </a>
